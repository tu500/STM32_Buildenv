...
#ifdef MQTT_DEBUG
#define MQTTDEBUG(...) printf(__VA_ARGS__)
#else
#define MQTTDEBUG(...)
#endif

#ifdef MQTT_PARSE_DEBUG
#define MQTTPARSEDEBUG(...) printf(__VA_ARGS__)
#else
...
#define U16_HI8(x)  ((uint8_t)((x) >> 8))
#define U16_LO8(x)  ((uint8_t)(x))

typedef uint16_t bufint_t;
typedef uint32_t timerint_t;
...
static bufint_t mqtt_send_buffer_last_length;   // length of last packet
                                                // (for uip-retransmit)
static bufint_t mqtt_send_buffer_current_head;  // current buffer head
static bufint_t mqtt_receive_buffer_length;     // length of data for received buffer
...
static timerint_t mqtt_last_out_activity;
static timerint_t mqtt_last_in_activity;
static bool mqtt_ping_outstanding;

// GENERAL STATE STRUCTURES

static mqtt_connection_config_t const *mqtt_con_config = NULL;
static mqtt_callback_config_t const *mqtt_callbacks[MQTT_CALLBACK_SLOTS]; // XXX
static mqtt_connection_state_t mqtt_con_state;
static uip_conn_t *mqtt_uip_conn;

static timerint_t mqtt_timer_counter = 0;
...
static uint8_t mqtt_buffer_write_length_field(uint8_t *buffer, uint16_t length);
...
static void mqtt_handle_packet(const void *data, uint8_t llen, uint16_t packet_length);
static uint8_t mqtt_parse_length_field(uint16_t *length, const void *buffer, uint16_t max_read);
...
static void mqtt_fire_publish_callback(char const *topic, uint16_t topic_length,
...
{
  memmove(mqtt_send_buffer + mqtt_send_buffer_current_head, data, length);
...
mqtt_buffer_write_string(char const *data)
{
  ...
  mqtt_send_buffer[mqtt_send_buffer_current_head-len-2] = U16_HI8(len);
  mqtt_send_buffer[mqtt_send_buffer_current_head-len-1] = U16_LO8(len);
}
...
mqtt_received_ack(void)
{
  ...
  if (mqtt_send_buffer_last_length > 0)
  {
    ...
    memcpy(
        mqtt_send_buffer,
        mqtt_send_buffer + mqtt_send_buffer_last_length,
...
mqtt_construct_connect_packet(void)
{
  ...
  // keep alive
  mqtt_send_buffer[mqtt_send_buffer_current_head++] = U16_HI8(MQTT_KEEPALIVE);
  mqtt_send_buffer[mqtt_send_buffer_current_head++] = U16_LO8(MQTT_KEEPALIVE);
  ...
  if (mqtt_con_config->will_topic)
  {
    ...
    mqtt_send_buffer[mqtt_send_buffer_current_head++] = U16_HI8(mqtt_con_config->will_message_length);
    mqtt_send_buffer[mqtt_send_buffer_current_head++] = U16_LO8(mqtt_con_config->will_message_length);
  ...
  uint16_t payload_length, bool retain)
{
  ...
  if (qos > 0)
  {
    mqtt_send_buffer[mqtt_send_buffer_current_head++] = U16_HI8(mqtt_next_msg_id);
    mqtt_send_buffer[mqtt_send_buffer_current_head++] = U16_LO8(mqtt_next_msg_id);
    make_new_message_id();
  }
...
mqtt_construct_ack_packet(uint8_t msg_type, uint16_t msgid)
{
  ...
  // message id
  mqtt_send_buffer[mqtt_send_buffer_current_head++] = U16_HI8(msgid);
  mqtt_send_buffer[mqtt_send_buffer_current_head++] = U16_LO8(msgid);
  ...
  while (bytes_read < uip_len)
  {
    ...
    if (!mqtt_receive_buffer_length)
    {
      ...
      if (llen == 0)
      {
        ...
        if (!mqtt_write_to_receive_buffer(uip_appdata + bytes_read, remaining_length))
        {
    ...
    else if (mqtt_receive_packet_length != 0)
    {
      ...
      {
        uint16_t fragment_length = mqtt_receive_packet_length - mqtt_receive_buffer_length;

        // write this last fragment to the receive buffer
        if (!mqtt_write_to_receive_buffer(uip_appdata + bytes_read, fragment_length))
        {
...
mqtt_fire_connack_callback(void)
{
  for (int i = 0; i < MQTT_CALLBACK_SLOTS; ++i)
  {
    ...
    {
      connack_callback cb = mqtt_callbacks[i]->connack_callback;
      if (cb != NULL)
        cb();
    }
  }
...
mqtt_fire_poll_callback(void)
{
  for (int i = 0; i < MQTT_CALLBACK_SLOTS; ++i)
  {
    if (mqtt_callbacks[i] != NULL)
    {
      poll_callback cb = mqtt_callbacks[i]->poll_callback;
      if (cb != NULL)
        cb();
    }
  }
...
mqtt_fire_close_callback(void)
{
  for (int i = 0; i < MQTT_CALLBACK_SLOTS; ++i)
  {
    if (mqtt_callbacks[i] != NULL)
    {
      close_callback cb = mqtt_callbacks[i]->close_callback;
      if (cb != NULL)
        cb();
    }
  }
    ...
    const void *payload, uint16_t payload_length, bool retained)
{
  for (int i = 0; i < MQTT_CALLBACK_SLOTS; ++i)
  {
    if (mqtt_callbacks[i] != NULL)
    {
      publish_callback cb = mqtt_callbacks[i]->publish_callback;
      if (cb != NULL)
...
mqtt_poll(void)
{
  if ((mqtt_timer_counter - mqtt_last_in_activity > MQTT_KEEPALIVE * TIMER_TICKS_PER_SECOND) ||
...
mqtt_init(void)
{
  if (!mqtt_uip_conn)
  {
    ...
    mqtt_uip_conn = uip_connect((uip_ipaddr_t *) & mqtt_con_config->target_ip, HTONS(1883));

    if (mqtt_uip_conn)
    {
      mqtt_uip_conn->appstate.callback = mqtt_main;
      MQTTDEBUG("connection initialized.\n");
    }
    else
    {
...
uint8_t
mqtt_register_callback(mqtt_callback_config_t const *callbacks)
{
  // search a free slot
